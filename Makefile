...
ifdef CI
    ...
    export VENV_PATH := /tmp/darca-llm-venv
    export POETRY_HOME := /tmp/poetry-llm-cache
    export POETRY_CONFIG_DIR := /tmp/poetry-llm-config
    export POETRY_CACHE_DIR := $(POETRY_HOME)
    export POETRY_VIRTUALENVS_PATH := $(VENV_PATH)
    export PYTHONPYCACHEPREFIX := /tmp/llm-pycache
    ...
    export PRE_COMMIT_HOME := /tmp/precommit-llm-cache
endif
